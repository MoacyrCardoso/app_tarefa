...
def adicionar_item_tarefa(tarefa):
    ...
    botao_editar = tk.Button(frame_tarefa, image=icon_editar, command=lambda f=frame_tarefa, l=label_tarefa:
    preparar_edicao(f, l), bg="white", relief=tk.FLAT)
    botao_editar.pack(side=tk.RIGHT, padx=5)

    botao_deletar = tk.Button(frame_tarefa, image=icon_deletar, command=lambda f=frame_tarefa: deletar_tarefa(f), 
...
# Crie os widgets de entrada e o botão para adicionar novas tarefas.
...
botao_adicionar = tk.Button(frame, text="Adicionar Tarefa",
command=Adicionar_tarefa,bg="#4CAF50", fg="white", height=1, width=15, font=("Robot", 11), relief=tk.FLAT)
...
scrollbar = ttk.Scrollbar(frame_lista_tarefas, 
...
# Por fim, execute o loop principal da aplicação para iniciar o aplicativo.
